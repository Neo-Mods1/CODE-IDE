...
import com.neo.ide.javac.services.compiler.JavacFlowListener
import com.neo.ide.javac.services.util.ReparserUtils
import com.neo.ide.javac.services.visitors.FindAnonymousVisitor
import com.neo.ide.javac.services.visitors.TranslateMethodPositionsVisitor
...
class PartialReparserImpl : PartialReparser {
  ...
  override fun reparseMethod(
    ...
    try {
      val l = com.neo.ide.javac.services.NBLog.instance(context)
  ...
  private fun newParser(
    ...
    val factory =
      com.neo.ide.javac.services.NBParserFactory.instance(context)
        as com.neo.ide.javac.services.NBParserFactory
    val scannerFactory = ScannerFactory.instance(context)
    val cancelService = com.neo.ide.javac.services.CancelService.instance(context)
    val lexer = scannerFactory.newScanner(buf, true)
    if (
      endPositions
        is com.neo.ide.javac.services.NBParserFactory.NBJavacParser.EndPosTableImpl
    ) {
      endPositions.resetErrorEndPos()
    }

    return object :
      com.neo.ide.javac.services.NBParserFactory.NBJavacParser(
        factory,
        lexer,
        true,
        false,
