...
import com.neo.ide.utils.IDEEnvironment;
import java.io.File;
...
public class IDEDocumentsProvider extends DocumentsProvider {
    ...
    private static final String TAG = "IDEDocumentsProvider";
    ...
    private File getBaseDir() {
        if (IDEEnvironment.HOME == null) {
            IDEEnvironment.init(getContext());
        }
        return IDEEnvironment.HOME;
    }
        ...
        while (!pending.isEmpty() && result.getCount() < MAX_SEARCH_RESULTS) {
            ...
            try {
                isInsideHome = file.getCanonicalPath().startsWith(
                    IDEEnvironment.HOME.getAbsolutePath());
            } catch (IOException e) {
                isInsideHome = true;
            }
            if (isInsideHome) {
